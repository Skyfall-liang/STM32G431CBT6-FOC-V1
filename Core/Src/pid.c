...
struct _PID pid_velocity_control = {0.5, 3, 0, 0, 0, 0, 0, 1000, 12};
struct _PID pid_current_control = {2, 6, 0, 0, 0, 0, 0, 1000, 6};

struct _PID pid_VC_velocity = {0.5, 3, 0, 0, 0, 0, 0, 1000, 2};
struct _PID pid_VC_current = {2, 10, 0, 0, 0, 0, 0, 1000, 12};

struct _PID pid_AVC_angle = {2, 0, 0, 0, 0, 0, 0, 1000, 200};
struct _PID pid_AVC_velocity = {0.5, 5, 0, 0, 0, 0, 0, 1000, 4};
struct _PID pid_AVC_current = {2, 6, 0, 0, 0, 0, 0, 1000, 12};

struct _PID pid_fw = {0.5, 2.0, 0, 0, 0, 0, 0, 5, 5};
struct _PID pid_CC_q = {0.5, 2.0, 0, 0, 0, 0, 0, 1000, 12};
struct _PID pid_CC_d = {0.5, 2.0, 0, 0, 0, 0, 0, 1000, 12};
...
float PID_Controller(struct _PID *pid, float error) 
{
    ...
    {
        Ts = (float)(pid->Timestamp_Last - now_us)/170*1e-6f;
    }
	else
    {
        Ts = (float)(0xFFFFFF - now_us + pid->Timestamp_Last)/170*1e-6f;
    }
	...
	integral = _constrain(integral, -(pid->output_limit)*0.9f, (pid->output_limit)*0.9f);
